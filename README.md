...
attribute grammar (`ui:state`, `ui:bind`, `ui:each`, `ui:fx`, `ui:use`), and a ~23 KB
runtime (8.7 KB gzipped) provides signals, keyed lists, and a closed effect-verb catalog —
no build step, no components, no vdom, no eval. A separate optional stylesheet supplies
the structural enhancer classes. Built agent-first: the authoring grammar ships with the
