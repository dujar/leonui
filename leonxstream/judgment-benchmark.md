# Judgment — leonxstream benchmark + skill audit (2026-09-25)
...
- Correctness assertions are literally one shared loop over all 4 frameworks (frameworks.test.ts:33–62) — identical by construction. Measurement is one shared `measure()` (line 69–87) plus a shared update loop — identical. No favoritism found in driver ordering (leonxstream runs first, others after — mild, symmetric).
- No internal-API advantage: leonxstream's bench driver uses `window.__ui.readPath/setPath`, which mirror the module's exported public API (`index.ts` exports `readPath, setPath`). React/Vue/Alpine drivers use their own public state APIs. Equivalent.
- Implementations are idiomatic: React `createElement` + keyed `useState` (production bundle — contains "Minified React error", no dev warnings), Vue `esm-bundler` + keyed runtime-compiled template (the symmetric choice vs leonxstream's runtime-compiled attributes), Alpine `x-for` keyed. `make(n)` id semantics differ (leonxstream restarts ids, others monotonic) but all ops create from an empty list, so keyed-diff work is equivalent.
- The generator is mechanical, not gamed: benchmark.md is produced from `results`/`sizes` in the same test run; artifact numbers match the table exactly; my rerun honestly crowned **vue** for create/replace where the committed run crowned leonxstream — it does not protect leonxstream.
- Mount (67.4 vs react 38.7 committed; 48.9 vs vue 28.1 rerun) is favorable *against* leonxstream — evidence of honesty, not spin.

[should-fix] Mount column is not comparable across pages. Quoted line: `**mount** (navigate → scaffold interactive)` and `Measurement: ... identical`. leonxstream.html and alpine.html stamp `__benchReadyAt` after **4 rAFs** (`mount(){ await settle(); }` + outer `await settle()`); react.html/vue.html after **2 rAFs** (`mount(){}` noop). That is ~2 frames (~16–33 ms in this headless setup) of protocol, not scaffold, charged to leonxstream/alpine — roughly the entire react-vs-leonxstream mount gap in both runs. Fix: give all four pages the identical stamp protocol (delete `await settle()` inside leonxstream/alpine `mount()`, or document it), or drop Mount from the table.
...
[should-fix] Winner bullets rank machine noise. Quoted: `- **create1000** → **leonxstream** (spread 26.0 ms)` — the spread is Alpine's 54.7; leonxstream vs react vs vue were 28.7/30.6/29.2 (1.9 ms apart), and my rerun of the same code flipped the crown to vue (29.2 vs 29.3). The same file's honest reading says "treat sub-10 ms gaps as machine noise" — the auto-generated bullets contradict the caveat one section above, and the committed artifact happens to flatter leonxstream. Fix (smallest): in the generator's `winner()` block, emit "→ winner" only when the winner beats the runner-up by > 5 ms; otherwise print "statistical tie".

[should-fix] The "Minified" column is false for the leonxstream row. Quoted header: `| Framework | Minified | Gzipped |`. `package.json` build is `bun build ./src/index.ts --outfile ./dist/leonxstream.js --target=browser` — no `--minify`; dist/leonxstream.js is readable with `// src/version.ts` comments, while the three vendor bundles are name-mangled minified production builds. The error direction is conservative (leonxstream's 38.8 KB is its *unminified* size), but the label is still wrong in a deliverable whose premise is honesty. Fix: add `--minify` to the build and rebuild before measuring, or relabel the column.
...
Nothing fabricated, nothing gamed, no favoritism to leonxstream (twice the generator recorded results against it). The failures are all small and all in the "presented fairness" layer, not the measurement or the runtime.
...
1. Mount protocol — FIXED. All four bench pages now stamp `__benchReadyAt` after the identical outer single 2-rAF settle (leonxstream/alpine `mount()` reduced to noop/`Alpine.start()`); methodology line updated ("identical 2-rAF stamp protocol on all four pages").
2. Winner bullets — FIXED. Generator emits a ranking only when the winner beats the runner-up by > 5 ms (`gap > 5` gate). Two independent regenerated runs produced: create1000 tie (1.6 / 0.9 ms), replace tie (0.2 / 0.8 ms), update saturated, remove → alpine (10.0 / 11.7 ms — a real gap, correctly still ranked). No leonxstream-favoring claims remain.
3. Minified column — FIXED. `--minify` in the build; dist/leonxstream.js is now 17,737 bytes; sizes.json leonxstream = 27,770 B / 9,379 gz = the table's 27.1 / 9.2 KB. Vendors unchanged (still minified prod). Header now true for every row.
...
- [note] Mount remains the noisiest column run-to-run (leonxstream 48.9 / 67.4 / 80.7 across my three runs; react 39.6 / 38.7 / 35.4) and leonxstream is always the first framework measured (cold first navigation). Protocol is now identical and the numbers shown are unflattering to leonxstream, so this is honesty-neutral — but rotating framework order or reporting mount as a range would remove the last variance artifact.
