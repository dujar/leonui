...
async function run(): Promise<void> {
  ...
  const dist = new URL('../dist/leonxstream.js', import.meta.url).pathname;
  ...
  const md = `# leonxstream benchmark — ${new Date().toISOString().slice(0, 10)}
...
- Implementations are idiomatic per framework: React \`createElement\` + keys, Vue \`ref\` + \`v-for\` keyed, Alpine \`x-for\` keyed, leonxstream \`ui:each\` + immutable \`set\` via its public API (\`__ui.setPath\` mirrors the module export).
...
(leonxstream size = runtime bundle + shipped stylesheet, the complete framework cost.)
...
- Bundle size is where the architecture shows: the whole leonxstream runtime ships in less code than any framework's runtime here — but it also does less (no scheduler, no suspense, no transition system).
