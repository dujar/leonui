...
test('docs: hello-world example actually runs', async () => {
  ...
  await p.waitFor(`document.querySelector('b').textContent === 'leonxstream'`);
  assert.equal(await p.eval<string>(`document.querySelector('p').textContent`), 'Hello, leonxstream!');
...
test('docs: every embedded example source is fetchable and boots the runtime', async () => {
  ...
  for (const e of examples) {
    ...
    assert.ok((await res.text()).includes('/dist/leonxstream.js'), `${e} loads the runtime`);
  }
});
