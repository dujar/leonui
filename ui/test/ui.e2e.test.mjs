...
const page = async path => browser.newPage(base).then(p => p.goto(path));
const text = (p, sel) => p.eval(`document.querySelector(${JSON.stringify(sel)})?.textContent`);
const html = (p, sel) => p.eval(`document.querySelector(${JSON.stringify(sel)})?.innerHTML`);
const style = (p, sel, prop) => p.eval(`getComputedStyle(document.querySelector(${JSON.stringify(sel)}))[${JSON.stringify(prop)}]`);
const exists = (p, sel) => p.eval(`!!document.querySelector(${JSON.stringify(sel)})`);
const click = (p, sel) => p.eval(`document.querySelector(${JSON.stringify(sel)}).click()`);
const rowsOf = (p, sel) => p.eval(`[...document.querySelector(${JSON.stringify(sel)}).children].filter(n => n.nodeType === 1).map(n => n.querySelector('.ui-list-title')?.textContent ?? n.textContent.trim())`);
const assertNoPageErrors = (p) => {
...
test('index renders and exposes the runtime', async () => {
...
test('binds: text aspect + expression concat', async () => {
  const p = await page('/pages/index.html');
  assert.equal(await text(p, 'p.ui-t-muted'), 'runtime v0.1.0 — component gallery');
  await p.close();
});

test('parser: whitelist rejects undeclared signals, forbidden calls, trailing input', async () => {
  const p = await page('/pages/index.html');
  const attempt = expr => p.eval(`(() => { try { window.__ui.safeEval(${JSON.stringify(expr)}, document.body); return 'accepted'; } catch (e) { return e.message; } })()`);
  assert.match(await attempt('undeclared_x + 1'), /undeclared signal/);
  assert.match(await attempt('fetch("http://evil")'), /unexpected/);           // rejected at parse
...
test('parser: arithmetic, comparison, ternary, spread, object literals', async () => {
  const p = await page('/pages/index.html');
  const run = expr => p.eval(`window.__ui.safeEval(${JSON.stringify(expr)}, document.body)`);
...
test('layout: stack gap scale maps tokens to computed styles', async () => {
...
test('layout: card variants + divider + spacer', async () => {
  const p = await page('/pages/layout.html');
...
test('content: text + badge variants carry their classes', async () => {
  ...
  assert.ok((await p.eval(`document.querySelector('#b-dynamic').className`)).includes('ui-b-brand'));
  await p.eval(`document.querySelector('#b-dynamic')`); // (scope: featured signal is page-level)
  await p.close();
});

test('content: icons inject the sprite and reference symbols', async () => {
  const p = await page('/pages/content.html');
...
test('content: image fallback class on broken src', async () => {
  const p = await page('/pages/content.html');
...
test('forms: two-way model syncs input -> signal -> echoes', async () => {
...
test('forms: select change updates signal', async () => {
  const p = await page('/pages/forms.html');
  await p.eval(`
...
test('forms: checkbox two-way bind + toggle verb', async () => {
  const p = await page('/pages/forms.html');
...
test('forms: button variants + disabled bind + reset/focus verbs', async () => {
  const p = await page('/pages/forms.html');
...
test('overlays: popover opens via platform invoker, placement wired, item runs verbs', async () => {
  ...
  assert.ok(await p.eval(`document.querySelector('#pop1').style.positionAnchor`), 'position-anchor wired');
  await click(p, '#pop1-item');
  assert.equal(await text(p, '#echo-gate'), 'gate = from-popover');
  await p.close();
});

test('overlays: modal dialog via commandfor show-modal/close', async () => {
  const p = await page('/pages/overlays.html');
...
test('overlays: toasts render text into the aria-live host', async () => {
  const p = await page('/pages/overlays.html');
...
test('overlays: failing call triggers onfail toast', async () => {
  const p = await page('/pages/overlays.html');
...
test('overlays: prompt + confirm + delay verbs', async () => {
  const p = await page('/pages/overlays.html');
...
test('navigation: tabs aria state, panels, arrow keys', async () => {
...
test('navigation: accordion details are exclusive via name group', async () => {
  const p = await page('/pages/navigation.html');
...
test('navigation: nav verb switches screens with view transition', async () => {
  const p = await page('/pages/navigation.html');
...
test('data: keyed list add/drop/rotate/remove keeps DOM aligned', async () => {
...
test('data: remote cell lifecycle loading -> ok + refetch', async () => {
  const p = await page('/pages/data.html');
...
test('data: remote error cell reports status=error', async () => {
  const p = await page('/pages/data.html');
  await p.waitFor(`document.querySelector('#rb-status').textContent === 'error'`);
  await p.close();
});

test('data: computed sortBy re-runs on dependency change', async () => {
  const p = await page('/pages/data.html');
...
test('data: failing PATCH rolls back the optimistic set + onfail toast', async () => {
  const p = await page('/pages/data.html');
...
test('data: scope shadowing — nearest ui:state wins', async () => {
  const p = await page('/pages/data.html');
...
test('css: tokens resolve as custom properties on :root', async () => {
  const p = await page('/pages/layout.html');
  const brand = await p.eval(`getComputedStyle(document.documentElement).getPropertyValue('--brand').trim()`);
  assert.ok(brand.length > 0, '--brand resolves');
  const gap2 = await p.eval(`getComputedStyle(document.documentElement).getPropertyValue('--ui-gap-2').trim()`);
  assert.equal(gap2, '4px');
  await p.close();
});

test('css: light-dark() flips with prefers-color-scheme (emulated dark)', async () => {
  const p = await page('/pages/layout.html');
...
test('css: cascade layers — app override beats ui.base', async () => {
  const p = await page('/pages/index.html');
...
test('inbox: remote load renders rows; count binds', async () => {
  ...
  assert.deepEqual(titles, ['Set up leonspace workspace', 'Write the interaction-tier spec', 'Run the verification-wedge tests']);
  await p.close();
});

test('inbox: add task — optimistic append, POST body, refetch reconciles ids', async () => {
  await resetServer();
  const p = await page('/pages/inbox.html');
  await p.waitFor(`document.querySelectorAll('#task-list .ui-list-item').length === 3`);
...
test('inbox: filter segments hide/show rows via binds', async () => {
  await resetServer();
  const p = await page('/pages/inbox.html');
  await p.waitFor(`document.querySelectorAll('#task-list .ui-list-item').length === 3`);
  const visibleTitles = () => p.eval(`[...document.querySelectorAll('#task-list .ui-list-item')].filter(li => !li.hidden).map(li => li.querySelector('.ui-list-title').textContent)`);
  ...
  assert.deepEqual(await visibleTitles(), ['Set up leonspace workspace']);
  await click(p, '#f-all');
  assert.equal((await visibleTitles()).length, 3);
  assert.ok((await p.eval(`document.querySelector('#f-all').className`)).includes('ui-btn-primary'), 'active segment styled');
  await p.close();
});

test('inbox: search filter via two-way model', async () => {
  await resetServer();
  const p = await page('/pages/inbox.html');
  await p.waitFor(`document.querySelectorAll('#task-list .ui-list-item').length === 3`);
...
test('inbox: toggle task — optimistic PATCH against the real API', async () => {
  await resetServer();
  const p = await page('/pages/inbox.html');
  await p.waitFor(`document.querySelectorAll('#task-list .ui-list-item').length === 3`);
...
test('inbox: delete task — optimistic removal + DELETE hits the server', async () => {
  await resetServer();
  const p = await page('/pages/inbox.html');
  await p.waitFor(`document.querySelectorAll('#task-list .ui-list-item').length === 3`);
...
test('inbox: no uncaught page errors across the whole journey', async () => {
  await resetServer();
  const p = await page('/pages/inbox.html');
  await p.waitFor(`document.querySelectorAll('#task-list .ui-list-item').length === 3`);
